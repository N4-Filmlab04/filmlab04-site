...
function chipRow(p) {
  const chips = [p.iso && `ISO ${p.iso}`, p.format, p.type].filter(Boolean).map(escapeHtml);
  ...
  return `
    <article class="product-card">
      <a href="product.html?id=${idAttr}" class="product-card-img">
        ${p.image ? `<img src="${escapeHtml(p.image)}" alt="${escapeHtml(p.brand)} ${escapeHtml(p.name)}">` : 'photo coming soon'}
      </a>
      <div class="product-card-body">
        ...
        <a href="product.html?id=${idAttr}"><h3 class="product-name">${escapeHtml(p.name)}</h3></a>
        <p class="product-tagline">${escapeHtml(p.tagline)}</p>
        ${chipRow(p)}
        <div class="product-card-footer">
          <span class="product-price">${escapeHtml(p.currency)}${p.price.toFixed(2)}</span>
          ${action}
        </div>
      </div>
    </article>`;
